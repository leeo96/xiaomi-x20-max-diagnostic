...
This project is intentionally **read-only**. The native MiIO implementation only exposes `get_properties`. It does not implement `set_properties`, movement, cleaning commands, or MIoT actions.

The MiIO token is entered locally on the Android device and is not transmitted to any external service by the app.

## Build

GitHub Actions builds a debug APK on every push to `main`. The artifact is named `X20-Max-Diagnostic-APK`.
